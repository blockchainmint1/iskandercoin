...
static bool HasMarkerUnsafe(const CTransactionRef& tx)
{
    ...
    const std::string strClassABTest("00146caf6c62bac6f70e0fc5de5f3f767cdb380aad0a"); // testnet exodus address scriptPubKey
...
int mastercore::GetEncodingClass(const CTransaction& tx, int nBlock)
{
    ...
    for (unsigned int n = 0; n < tx.vout.size(); ++n) {
        ...
        if (!IsAllowedOutputType((int)outType, nBlock)) {
            continue;
        }
        ...
        if (outType == TxoutType::MULTISIG) {
            hasMultisig = true;
        }
        if (outType == TxoutType::NULL_DATA) {
...
static bool FillTxInputCache(const CTransaction& tx, const std::shared_ptr<std::map<COutPoint, Coin>> removedCoins)
{
    ...
    for (std::vector<CTxIn>::const_iterator it = tx.vin.begin(); it != tx.vin.end(); ++it) {
        ...
        if (removedCoins && removedCoins->find(txIn.prevout) != removedCoins->end()) {
            ...
            BlockMap::iterator bit = g_chainman.m_blockman.m_block_index.find(hashBlock);
            newcoin.nHeight = bit != g_chainman.m_blockman.m_block_index.end() ? bit->second->nHeight : 1;
        } else {
            return false;
        }
...
static int parseTransaction(bool bRPConly, const CTransaction& wtx, int nBlock, unsigned int idx, CMPTransaction& mp_tx, unsigned int nTime, const std::shared_ptr<std::map<COutPoint, Coin>> removedCoins = nullptr)
{
    ...
    if (omniClass != OMNI_CLASS_C)
    {
        ...
        for (unsigned int i = 0; i < wtx.vin.size(); ++i) {
            ...
            if (!IsAllowedInputType((int)whichType, nBlock)) {
                return -105;
            }
    ...
    for (unsigned int n = 0; n < wtx.vout.size(); ++n) {
        ...
        if (!IsAllowedOutputType((int)whichType, nBlock)) {
            continue;
        }
        CTxDestination dest;
...
int mastercore_init()
{
...

    {
        ...
        if (inconsistentDb) {
            ...
            bilingual_str bAlert = Untranslated(strAlert);
            DoWarning(bAlert);
        }
...
bool mastercore::UseEncodingClassC(size_t nDataSize)
{
    ...
    if (!IsAllowedOutputType((int)(TxoutType::NULL_DATA), nBlockNow)) {
        fDataEnabled = false;
    }
