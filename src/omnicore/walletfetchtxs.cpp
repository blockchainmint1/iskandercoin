...
std::map<std::string, uint256> FetchWalletOmniTransactions(interfaces::Wallet& iWallet, unsigned int count, int startBlock, int endBlock)
{
    ...
    std::multimap<int64_t, const WalletTxRecord*> txOrdered;
    const std::vector<WalletTxRecord>& transactions = iWallet.getWalletTxs();
    for (const auto& transaction : transactions)
        txOrdered.insert(std::make_pair(transaction.GetWTX().nOrderPos, &transaction));

    // Iterate backwards through wallet transactions until we have count items to return:
    for (std::multimap<int64_t, const WalletTxRecord*>::reverse_iterator it = txOrdered.rbegin(); it != txOrdered.rend(); ++it) {
        const WalletTxRecord* pwtx = it->second;
        const uint256& txHash = pwtx->GetTxHash();
        {
            LOCK(cs_tally);
            if (!pDbTransactionList->exists(txHash)) continue;
        }
        const uint256& blockHash = pwtx->GetWTX().m_confirm.hashBlock;
    ...
    for (PendingMap::const_iterator it = my_pending.begin(); it != my_pending.end(); ++it) {
        ...
        {
            for (const auto& transaction : transactions)
                if (transaction.GetTxHash() == txHash)
                    blockPosition = transaction.GetWTX().nOrderPos;
        }
        std::string sortKey = strprintf("%06d%010d", blockHeight, blockPosition);
        mapResponse.insert(std::make_pair(sortKey, txHash));
    }
