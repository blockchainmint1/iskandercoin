...
static UniValue omni_funded_send(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 5)
        throw runtime_error(
            RPCHelpMan{"omni_funded_send",
               ...
               "\nAll litecoins from the sender are consumed and if there are litecoins missing, they are taken from the specified fee source. Change is sent to the fee source!\n",
               {
    ...
    uint256 retTxid;
    int result = CreateFundedTransaction(node, fromAddress, toAddress, feeAddress, payload, retTxid, pwallet.get());
    if (result != 0) {
        ...
        throw runtime_error(
            RPCHelpMan{"omni_sendrawtx",
               "\nBroadcasts a raw Omni Layer transaction.\n",
               {
                   ...
                   {"referenceamount", RPCArg::Type::STR, RPCArg::Optional::OMITTED, "a litecoin amount that is sent to the receiver (minimal by default)\n"},
               },
               RPCResult{
                    RPCResult::Type::STR_HEX, "", "The hex-encoded transaction hash."
                },
    ...
    if (request.fHelp || request.params.size() != 7)
        throw runtime_error(
            RPCHelpMan{"omni_senddexsell",
               "\nPlace, update or cancel a sell offer on the distributed token/LTC exchange.\n",
               {
                   {"fromaddress", RPCArg::Type::STR, RPCArg::Optional::NO, "the address to send from\n"},
                   ...
                   {"amountdesired", RPCArg::Type::STR, RPCArg::Optional::NO, "the amount of litecoins desired\n"},
    ...
    if (request.fHelp || request.params.size() != 6)
        throw runtime_error(
            RPCHelpMan{"omni_sendnewdexorder",
               "\nCreates a new sell offer on the distributed token/LTC exchange.\n",
               {
                   {"fromaddress", RPCArg::Type::STR, RPCArg::Optional::NO, "the address to send from\n"},
                   {"propertyidforsale", RPCArg::Type::NUM, RPCArg::Optional::NO, "the identifier of the tokens to list for sale\n"},
                   {"amountforsale", RPCArg::Type::STR, RPCArg::Optional::NO, "the amount of tokens to list for sale\n"},
    ...
    if (request.fHelp || request.params.size() != 6)
        throw runtime_error(
            RPCHelpMan{"omni_sendupdatedexorder",
               "\nUpdates an existing sell offer on the distributed token/LTC exchange.\n",
               {
                   {"fromaddress", RPCArg::Type::STR, RPCArg::Optional::NO, "the address to send from\n"},
                   ...
                   {"amountdesired", RPCArg::Type::STR, RPCArg::Optional::NO, "the new amount of litecoins desired\n"},
    ...
    if (request.fHelp || request.params.size() != 2)
        throw runtime_error(
            RPCHelpMan{"omni_sendcanceldexorder",
               "\nCancels existing sell offer on the distributed token/LTC exchange.\n",
               {
                   {"fromaddress", RPCArg::Type::STR, RPCArg::Optional::NO, "the address to send from\n"},
...
static UniValue omni_senddexpay(const JSONRPCRequest& request)
{
        ...
        throw runtime_error(
            RPCHelpMan{"omni_senddexpay",
               "\nCreate and broadcast payment for an accept offer.\n",
               {
                   ...
                   {"toaddress", RPCArg::Type::STR, RPCArg::Optional::NO, "the address of the seller\n"},
                   {"propertyid", RPCArg::Type::NUM, RPCArg::Optional::NO, "the identifier of the token to purchase\n"},
                   {"amount", RPCArg::Type::STR, RPCArg::Optional::NO, "the Litecoin amount to send\n"},
               },
               RPCResult{
                    RPCResult::Type::STR_HEX, "", "The hex-encoded transaction hash."
                },
    ...
    CTxDestination dest = DecodeDestination(sellerAddress);
    ...
    // Get accept offer and make sure buyer is not trying to overpay
    {
        ...
        if (nAmount > amountToPayInBTC) {
            throw JSONRPCError(RPC_MISC_ERROR, strprintf("Paying more than required: %lld LTC to pay for %lld tokens", FormatMoney(amountToPayInBTC), FormatMP(propertyId, amountAccepted)));
        }

        if (!isPropertyDivisible(propertyId) && nAmount < amountToPayInBTC) {
            throw JSONRPCError(RPC_MISC_ERROR, strprintf("Paying less than required: %lld LTC to pay for %lld tokens", FormatMoney(amountToPayInBTC), FormatMP(propertyId, amountAccepted)));
        }
    }

    // Set the change address to the sender
    CCoinControl coinControl;
    coinControl.destChange = buyerDest;
    ...
    if (0 > mastercore::SelectCoins(*pwallet, buyerAddress, coinControl, nAmount + dust)) {
        throw JSONRPCError(RPC_WALLET_INSUFFICIENT_FUNDS, "Availabe funds in buyer address less than required to pay for accept offer, Exodus amount and fee");
    }

    // Make sure that we have inputs selected.
    if (!coinControl.HasSelected()) {
        throw JSONRPCError(RPC_WALLET_ERROR, "No outputs were selected to pay for the accept offer");
    }
    ...
    if (!wtxNew) {
        throw JSONRPCError(RPC_WALLET_ERROR, strprintf("ERROR: wallet transaction creation failed: %s\n", strFailReason.original));
    }

    // Commit the transaction to the wallet and broadcast
    std::vector<ReserveDestination*> reserved_keys;
    pwallet->commitTransaction(wtxNew, {}, {}, reserved_keys);
    ...
    if (request.fHelp || request.params.size() != 14)
        throw runtime_error(
            RPCHelpMan{"omni_sendissuancecrowdsale",
               ...
               RPCExamples{
                   HelpExampleCli("omni_sendissuancecrowdsale", "\"3JYd75REX3HXn1vAU83YuGfmiPXW7BpYXo\" 2 1 0 \"Companies\" \"Litecoin Mining\" \"Quantum Miner\" \"\" \"\" 2 \"100\" 1483228800 30 2")
                   + HelpExampleRpc("omni_sendissuancecrowdsale", "\"3JYd75REX3HXn1vAU83YuGfmiPXW7BpYXo\", 2, 1, 0, \"Companies\", \"Litecoin Mining\", \"Quantum Miner\", \"\", \"\", 2, \"100\", 1483228800, 30, 2")
               }
            }.ToString());
    ...
    if (request.fHelp || request.params.size() != 10)
        throw runtime_error(
            RPCHelpMan{"omni_sendissuancefixed",
               ...
               RPCExamples{
                   HelpExampleCli("omni_sendissuancefixed", "\"3Ck2kEGLJtZw9ENj2tameMCtS3HB7uRar3\" 2 1 0 \"Companies\" \"Litecoin Mining\" \"Quantum Miner\" \"\" \"\" \"1000000\"")
                   + HelpExampleRpc("omni_sendissuancefixed", "\"3Ck2kEGLJtZw9ENj2tameMCtS3HB7uRar3\", 2, 1, 0, \"Companies\", \"Litecoin Mining\", \"Quantum Miner\", \"\", \"\", \"1000000\"")
               }
            }.ToString());
    ...
    if (request.fHelp || request.params.size() != 9)
        throw runtime_error(
            RPCHelpMan{"omni_sendissuancemanaged",
               ...
               RPCExamples{
                   HelpExampleCli("omni_sendissuancemanaged", "\"3HsJvhr9qzgRe3ss97b1QHs38rmaLExLcH\" 2 1 0 \"Companies\" \"Litecoin Mining\" \"Quantum Miner\" \"\" \"\"")
                   + HelpExampleRpc("omni_sendissuancemanaged", "\"3HsJvhr9qzgRe3ss97b1QHs38rmaLExLcH\", 2, 1, 0, \"Companies\", \"Litecoin Mining\", \"Quantum Miner\", \"\", \"\"")
               }
            }.ToString());
