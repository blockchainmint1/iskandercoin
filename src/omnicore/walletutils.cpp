...
bool AddressToPubKey(const interfaces::Wallet* iWallet, const std::string& key, CPubKey& pubKey)
{
    ...
    if (IsValidDestination(dest)) {
        ...
        DestinationAddr destAddr(dest);
        const CScript& scriptPubKey = destAddr.GetScript();
        if (!iWallet->getPubKey(scriptPubKey, keyID, pubKey)) {
...
bool CheckInput(const CTxOut& txOut, int nHeight, CTxDestination& dest)
{
    ...
    if (!IsAllowedInputType((int)whichType, nHeight)) {
        return false;
    }
