        ...
        std::vector<COutPoint>& retLockedCoins)
{
    ...
    for (COutputCoin& output : vCoins) {
        const CWalletTx* walletTx = output.GetWalletTx();
        CTxDestination address;
        bool fValidAddress = output.GetDestination(address);

        // don't lock specified coins, but any other
        if (fValidAddress && destinations.count(address)) {
            continue;
        }
        COutPoint outpointLocked(walletTx->GetHash(), output.GetDepth());
        ...
        interfaces::Wallet* iWallet)
{
    ...
    if (fSuccess && nChangePosRet == -1 && receiverAddress == feeAddress) {
        fSuccess = false;
        strFailReason = _("send to self without change");
    }

    if (fSuccess && wtxNew->vout.size() > max_outputs)
    {
        strFailReason = _("more outputs than expected");
    ...
    if (fSuccess)
    {
        std::vector<OutputIndex> vSelectedInputs;
        coinControl.ListSelected(vSelectedInputs);

        // add previously selected coins
        for(const OutputIndex& outputIndex : vSelectedInputs) {
            if (auto outpoint = boost::get<COutPoint>(&outputIndex)) {
    ...
    {
        CTxMemPool& mempool = ::ChainstateActive().m_mempool;
        CAmount fee{DEFAULT_TRANSACTION_MAXFEE};
        LOCK(cs_main);
        if (!AcceptToMemoryPool(mempool, state, ctx, nullptr, false, false, &fee)) {
    ...
    int64_t virtual_size = GetVirtualTransactionSize(*ctx);
    CAmount max_raw_tx_fee = DEFAULT_MAX_RAW_TX_FEE_RATE.GetTotalFee(virtual_size, ctx->mweb_tx.GetMWEBWeight());
    const TransactionError err = BroadcastTransaction(node, ctx, err_string, max_raw_tx_fee, true, true);
