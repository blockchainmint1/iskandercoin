...
{
    LOCK(cs_tally);
    PrintToConsole("called LOCK and calling isPropertyIDValid\n");
...
{
    if ((0.01 * COIN) < amount) {
        throw JSONRPCError(RPC_TYPE_ERROR, "Reference amount higher is than 0.01 LTC");
    }
}
...
void RequireSaneDExFee(const std::string& address, uint32_t propertyId)
{
    ...
    if (poffer->getMinFee() > 1000000) {
        throw JSONRPCError(RPC_TYPE_ERROR, "Minimum accept fee is higher than 0.02 LTC (use override = true to continue)");
    }
}
