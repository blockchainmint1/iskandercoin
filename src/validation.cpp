    ...
    for (const CTxInput& txin : tx.GetInputs())
    {
        ...
        if (ptxConflicting) {
            if (!setConflicts.count(ptxConflicting->GetHash()))
            {
                ...
                // Iskander: Only support BIP125 RBF when -mempoolreplacement arg is set
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    CAmount nSubsidy = 312 * COIN; // 312 ISK block reward
    nSubsidy >>= halvings;
    return nSubsidy;
}
...
{
    AbortNode(strMessage, Untranslated(userMessage));
}
...
void DoWarning(bilingual_str& warning)
{
    ...
    if (!::ChainstateActive().IsInitialBlockDownload())
    {
        ...
        for (int bit = 0; bit < VERSIONBITS_NUM_BITS; bit++) {
            ...
            if (state == ThresholdState::ACTIVE || state == ThresholdState::LOCKED_IN) {
                bilingual_str warning = strprintf(_("Warning: unknown new rules activated (versionbit %i)"), bit);
