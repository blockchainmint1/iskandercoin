TexitCoin Core version 0.18.1 is now available from:

  <https://download.texitcoin.org/texitcoin-0.18.1/>
...
`/Applications/TexitCoin-Qt` (on Mac) or `texitcoind`/`texitcoin-qt` (on
Linux).
...
TexitCoin Core is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not
recommended to use TexitCoin Core on unsupported systems.
...
10.10. Additionally, TexitCoin Core does not yet change appearance when
...
- The `rpcallowip` option can no longer be used to automatically listen
  ...
  [...] -p 127.0.0.1:15739:15739` (this is an extra `:15739` over the
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](mailto:contact@texitcoin.org) the TexitCoin Core
  project to let us know about their use-case, as this feature could be
  deprecated in the future).

Wallet changes
--------------
When creating a transaction with a fee above `-maxtxfee` (default 0.1 TXC),
the RPC commands `walletcreatefundedpsbt` and  `fundrawtransaction` will now fail
instead of rounding down the fee. Beware that the `feeRate` argument is specified
in TXC per kilobyte, not litoshi per byte.
...
- A new [document](/blob/master/doc/texitcoin-conf.md)
  about the `texitcoin.conf` file describes how to use it to configure
  TexitCoin Core.

- A new document introduces TexitCoin Core's BIP174 [Partially-Signed
  TexitCoin Transactions
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent TexitCoin-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  TexitCoin Core to libssl vulnerabilities in the past, builders who don't
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when TexitCoin Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of TexitCoin Core, subsequently importing
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart texitcoind with the `-deprecatedrpc=generate`
...
- A new `texitcoin-wallet` tool is now distributed alongside TexitCoin
...
This section describes planned changes to TexitCoin Core that may affect
other TexitCoin software and services.

- Since version 0.16.0, TexitCoin Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with TexitCoin Core 0.20 (expected about
  a year after 0.18), TexitCoin Core will default to native segwit
  ...
  to bech32 addresses, and if the TexitCoin Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in TexitCoin Core 0.20.
  ...
  set the `addresstype=bech32` configuration option in any TexitCoin Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, TexitCoin Core
...
- A sub-project of TexitCoin Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with TexitCoin Core.  See their [project
...
- This release changes the Random Number Generator (RNG) used from
  OpenSSL to TexitCoin Core's own implementation, although entropy
  gathered by TexitCoin Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves TexitCoin Core a
...
- On macOS, TexitCoin Core now opts out of application CPU throttling
...
- #15739 Let wallet `importmulti` RPC accept labels for standard scriptPubKeys (ryanofsky)
