Iskander Core version 0.18.1 is now available from:

  <https://download.iskander.org/iskander-0.18.1/>
...
`/Applications/Iskander-Qt` (on Mac) or `iskanderd`/`iskander-qt` (on
Linux).
...
Iskander Core is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not
recommended to use Iskander Core on unsupported systems.
...
10.10. Additionally, Iskander Core does not yet change appearance when
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](mailto:contact@iskander.org) the Iskander Core
  project to let us know about their use-case, as this feature could be
  deprecated in the future).

Wallet changes
--------------
When creating a transaction with a fee above `-maxtxfee` (default 0.1 ISK),
the RPC commands `walletcreatefundedpsbt` and  `fundrawtransaction` will now fail
instead of rounding down the fee. Beware that the `feeRate` argument is specified
in ISK per kilobyte, not litoshi per byte.
...
- A new [document](/blob/master/doc/iskander-conf.md)
  about the `iskander.conf` file describes how to use it to configure
  Iskander Core.

- A new document introduces Iskander Core's BIP174 [Partially-Signed
  Iskander Transactions
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Iskander-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Iskander Core to libssl vulnerabilities in the past, builders who don't
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Iskander Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Iskander Core, subsequently importing
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart iskanderd with the `-deprecatedrpc=generate`
...
- A new `iskander-wallet` tool is now distributed alongside Iskander
...
This section describes planned changes to Iskander Core that may affect
other Iskander software and services.

- Since version 0.16.0, Iskander Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Iskander Core 0.20 (expected about
  a year after 0.18), Iskander Core will default to native segwit
  ...
  to bech32 addresses, and if the Iskander Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Iskander Core 0.20.
  ...
  set the `addresstype=bech32` configuration option in any Iskander Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Iskander Core
...
- A sub-project of Iskander Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Iskander Core.  See their [project
...
- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Iskander Core's own implementation, although entropy
  gathered by Iskander Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Iskander Core a
...
- On macOS, Iskander Core now opts out of application CPU throttling
