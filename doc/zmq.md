...
The Iskander Core daemon can be configured to act as a trusted "border
router", implementing the iskander wire protocol and relay, making
...
The ZeroMQ feature in Iskander Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building iskanderd:
...
For instance:
    ...
    $ iskanderd -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/iskanderd.tx.raw \
...
These options can also be provided in iskander.conf.
...
From the perspective of iskanderd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into iskanderd directly. Furthermore, no information is
...
using. Iskanderd appends an up-counting sequence number to each
