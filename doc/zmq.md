...
The TexitCoin Core daemon can be configured to act as a trusted "border
router", implementing the texitcoin wire protocol and relay, making
...
The ZeroMQ feature in TexitCoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building texitcoind:
...
For instance:
    ...
    $ texitcoind -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/texitcoind.tx.raw \
...
These options can also be provided in texitcoin.conf.
...
From the perspective of texitcoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into texitcoind directly. Furthermore, no information is
...
using. TexitCoind appends an up-counting sequence number to each
